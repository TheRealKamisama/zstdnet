...
本仓库是 **ZstdNet 的向后移植（Backport）Fork**，在保留现代版本模块的基础上，将兼容范围向后扩展至 **Minecraft 1.12.2 / Cleanroom / Java 25**。当前 Cleanroom 模块版本为 `1.4.7-cleanroom-alpha.1`，用户已确认实际进服与 Zstd 压缩正常。

**下载：[本 Fork 的 GitHub Releases](https://github.com/TheRealKamisama/zstdnet-cleanroom/releases)**。默认维护分支是 `cleanroom-1.12.2`，`main` 保留上游基线。带版本的发布标签由 GitHub Actions 构建、验证并发布；alpha 版本标记为 Pre-release。
...
服务端会自动保存每日流量、5 分钟时间序列和每次服务器运行记录，并分别统计 TCP、UDP、入站、出站以及压缩前后的数据量。统计文件位于 `config/zstdnet/stats/`。

拥有 2 级命令权限的玩家可以在已安装模组的客户端执行：
...
服务端只通过游戏连接返回聚合 JSON；客户端会在 `.minecraft/config/zstdnet/reports/` 生成自包含 HTML，并在聊天栏显示可点击的“打开统计面板”。

报告网页默认提供全部、过去 1 小时、过去 6 小时和过去 12 小时筛选，也支持报告范围内任意自定义起止时间；`/zstdreport 30d` 报告还会额外提供过去 1 天和过去 7 天。趋势图、流量摘要、方向构成和带宽参考会同步显示所选时间段的数据。

面板支持深色/浅色模式，并内置多种主题色。每套主题都为入站和出站使用高对比配色；带宽参考同时显示 `KiB/s` 与 `Mbps`，其中 `1 Mbps = 1,000,000 bit/s`。主题选择会保存在浏览器本地。
