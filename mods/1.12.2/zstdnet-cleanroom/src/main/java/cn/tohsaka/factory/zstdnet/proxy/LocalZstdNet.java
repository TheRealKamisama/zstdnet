...
public final class LocalZstdNet {
    ...
    private static boolean probeRawStatus(String remoteHost, int remotePort) {
        try (Socket probe = new Socket()) {
            ...
            byte[] firstPacket = PacketIo.extractPacketPayload(PacketIo.readPacketWire(probe.getInputStream(), 1024 * 1024));
            ...
            try {
                ...
                if (payloadLength == null) {
                    ...
                    if (packetLength != null) {
                        ...
                        if (payloadLength > 2048) throw new IOException("handshake exceeds 2048 bytes");
    ...
    private static byte[] rewriteHandshakeDestination(byte[] handshakePayload, String host, int port) {
        ...
        if (Integer.valueOf(2).equals(extractHandshakeNextState(handshakePayload))) {
            hostSuffix = ensureForgeHandshakeSuffix(hostSuffix);
        }
        LOGGER.info(
    ...
    static String ensureForgeHandshakeSuffix(String hostSuffix) {
        String suffix = hostSuffix == null ? "" : hostSuffix;
        suffix = suffix.replace("\0FML2\0", "\0FML\0").replace("\0FML3\0", "\0FML\0");
        return suffix.contains("\0FML\0") ? suffix : suffix + "\0FML\0";
    }
    ...
    private static final class ProxyStats {
        ...
        void track(Socket socket) {
            ...
            sockets.add(socket);
            if (closed) closeQuietly(socket);
        }
        void closeSockets() {
            closed = true;
            sockets.forEach(LocalZstdNet::closeQuietly);
            sockets.clear();
        }
